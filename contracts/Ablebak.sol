...
/////////////////
	...
	function initAgent(address _address) public returns (bool) {
		...
		bytes32 keyid_;
		bytes32 uuid_;
		(,,,,,keyid_,uuid_,,) = Doers(_address).getDoer(); 
		agents[_address] = 
		Agent({
			keyId: keyid_, 
			state: IS.INACTIVE, 
...
/////////////////
	...
	function callBack(bytes32 _data) external onlyCreator returns (bool) {
		require (callBackData[_data] == false);
		callBackState[true] = callBackState[callBackData[_data]];
		delete callBackState[callBackData[_data]];
		callBackData[_data] = true;
		return true;
	}
	...
	uint _year) external onlyDoer {
		...
		callBackData[callid] = false;
		callBackState[callBackData[callid]] = BE.QUALIFICATION;
	...
	external onlyDoer {
		bytes32 callid = keccak256(msg.data);
		Doers(creator).setReputation(Reputation({
			refMSD: _refMSD, 
			refRank: _refRank, 
			refSigned: _refSigned, 
			refSigs: _refSigs, 
			refTrust: _refTrust}));
		callBackData[callid] = false;
		callBackState[callBackData[callid]] = BE.REPUTATION;
		LogSetbdi(this,msg.sender,tx.origin,Iam.keyid,Iam.uuid,callid);
	}

	function setbdi(bytes32 _talent) external onlyDoer {
		bytes32 callid = keccak256(msg.data);
		Doers(creator).setTalent(_talent);
		callBackData[callid] = false;
		callBackState[callBackData[callid]] = BE.TALENT;
		LogSetbdi(this,msg.sender,tx.origin,Iam.keyid,Iam.uuid,callid);
	}

	function setQualification(KBase _kbase, Qualification _qualification, uint _year) public onlyCreator {
		require(callBackState[false] == BE.QUALIFICATION);
		callBackState[false] = BE.NULL;
		...
		callBackState[true] = BE.QUALIFICATION;
		updateIndex();
	}

	function setReputation(Reputation _data) public onlyCreator {
		require(callBackState[false] == BE.REPUTATION);
		callBackState[false] = BE.NULL;
		reputation = _data;
		bdi.beliefs.merits.reputation = reputation.refTrust;
		bytes32 callid = keccak256(msg.data); //  keccak256(_reputation);
		callBackState[true] = BE.REPUTATION;
		updateIndex();
	}

	function setTalent(bytes32 _talent) public onlyCreator {
		require(callBackState[false] == BE.TALENT);
		callBackState[false] = BE.NULL;
		...
		bytes32 callid = keccak256(msg.data); // keccak256(_talent);
		callBackState[true] = BE.TALENT;
		updateIndex();
	}
