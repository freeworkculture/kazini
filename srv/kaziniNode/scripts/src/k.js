; (function () {
	...
	window.kaziniApp.prototype.buildData = function (result) {
		...
		*/
		...
		if (this.bCanEdit) {
			// add new dish callcbs(item)back
		...
		function testETH() {
			...
			import { Web3Wrapper } from '../../lib/@0x/web3-wrapper';
			
			import { NETWORK_CONFIGS, TX_DEFAULTS } from './zeroex/configs';
			import { DECIMALS, NULL_ADDRESS } from './zeroex/constants';
			import { getContractAddressesForNetwork, getContractWrappersConfig } from './zeroex/contracts';
			import { PrintUtils } from './zeroex/print_utils';
			import { providerEngine } from './zeroex/provider_engine';
			import { getRandomFutureDateInSeconds } from './zeroex/utils';
			...
			export async function scenarioAsync()/*: Promise<void>*/ {
				...
				const contractWrappers = new ZeroEx.ContractWrappers(providerEngine, getContractWrappersConfig(NETWORK_CONFIGS.networkId));
				...
				const makerAssetAmount = Web3Wrapper.toBaseUnitAmount(new bignumber.BigNumber(5), DECIMALS);
				// the amount the maker wants of taker asset
				const takerAssetAmount = Web3Wrapper.toBaseUnitAmount(new bignumber.BigNumber(0.1), DECIMALS);
				// the amount of fees the maker pays in ZRX
				const makerFee = Web3Wrapper.toBaseUnitAmount(new bignumber.BigNumber(0.01), DECIMALS);
				// the amount of fees the taker pays in ZRX
				const takerFee = Web3Wrapper.toBaseUnitAmount(new bignumber.BigNumber(0.01), DECIMALS);
				...
				const takerSignatureHex = await ZeroEx.signatureUtils.ecSignHashAsync(providerEngine, executeTransactionHex, taker);
